:- module(term_util, [reverse_conjunction/2]).

gather_conjunction(Expr, Terms) :-
    Expr =.. [',', A, B] ->
    (
        not(functor(B, ',', _)) -> Terms = [A, B];
        gather_conjunction(B, RestTerms),
        Terms = [A | RestTerms]
    );

    Terms = [Expr].

build_conjunction(A, [A]).
build_conjunction((A,B), [A,B]).
build_conjunction(Expr, [A | Rest]) :-
    build_conjunction(RestExpr, Rest),
    Expr =.. [',', A, RestExpr].

reverse_conjunction(Expr, Reversed) :-
    gather_conjunction(Expr, Terms),
    reverse(Terms, ReversedTerms),
    build_conjunction(Reversed, ReversedTerms).
